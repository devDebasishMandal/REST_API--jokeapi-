...
app.patch("/jokes/:id", (req, res) => {
 ...
 const replacedJoke={
  id:id,
  //if either of them is  avalible use it
  jokeText: req.body.text ||existingJoke.jokeText,
  jokeType:req.body.type  ||existingJoke.jokeType
 };
//  find the id of the replecement joke and update the joke 
 const searchIndex = jokes.findIndex((joke)=> joke.id===id); 
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  // if the index actully exists then 
  if(searchIndex > -1) {
    // remove the index -- 1  here removes only one element
    // from the searchIndex 
jokes.splice(searchIndex,1);
res.sendStatus(200)
  }else{
    res.status(404).json({error:`Joke with ${id} not found.
    No jokes were deletd.`});
  }
